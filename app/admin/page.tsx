...
export default function AdminPage() {
  ...
  async function load() {
    ...
    setStats({ total: list.length, active, pending, pool: active * 20 })
    setLoading(false)
  }
  ...
  useEffect(() => {
    ...
    // Realtime updates when new users register
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-400 text-sm mt-1">Activate participants after receiving their Twint payment of CHF 20.</p>
        </div>

        {/* Stats */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
          {[
            { label:'Total registered', val: stats.total,        color:'text-gray-900' },
        ...
        {/* Prize breakdown */}
        <div className="bg-gray-900 rounded-2xl p-5 mb-8 flex gap-6 items-center flex-wrap">
          <div className="text-white flex-1">
            ...
            <div className="text-gray-400 text-xs mt-1">{stats.active} paid × CHF 20</div>
          </div>
        ...
        <div className="mb-8">
          ...
          {loading ? (
            <div className="bg-white border border-gray-100 rounded-2xl p-6 text-center text-gray-400 text-sm">Loading…</div>
          ) : pending.length === 0 ? (
            ...
            <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm">
              {pending.map((u, i) => (
                <div key={u.id} className={`flex items-center gap-4 p-4 ${i < pending.length-1 ? 'border-b border-gray-50' : ''}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-semibold text-gray-900 text-sm">{u.display_name}</div>
                    <div className="text-xs text-gray-400">
                      Registered {new Date(u.created_at).toLocaleDateString('en-CH', { day:'numeric', month:'short', hour:'2-digit', minute:'2-digit' })}
                    </div>
                  </div>
                  <div className="text-xs bg-yellow-50 border border-yellow-200 text-yellow-700 font-semibold px-3 py-1 rounded-full flex-shrink-0">⏳ Pending</div>
                  <button
                    onClick={() => activate(u.id)}
                    disabled={activating === u.id}
                    className="bg-green-600 hover:bg-green-700 text-white text-xs font-bold px-4 py-2 rounded-xl transition-colors flex-shrink-0"
                  >
        ...
        <div>
          ...
          <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm">
            {active.length === 0 ? (
              <div className="p-6 text-center text-gray-400 text-sm">No active participants yet</div>
            ) : active.map((u, i) => (
              <div key={u.id} className={`flex items-center gap-4 p-4 ${i < active.length-1 ? 'border-b border-gray-50' : ''}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-semibold text-gray-900 text-sm">{u.display_name}</div>
                  <div className="text-xs text-gray-400">
                    Joined {new Date(u.created_at).toLocaleDateString('en-CH', { day:'numeric', month:'short' })}
                  </div>
                </div>
                <div className="text-xs bg-green-50 border border-green-200 text-green-700 font-semibold px-3 py-1 rounded-full flex-shrink-0">✓ Active</div>
                <button
                  onClick={() => deactivate(u.id)}
                  disabled={activating === u.id}
                  className="text-xs text-gray-400 hover:text-red-500 font-medium px-3 py-2 rounded-xl transition-colors flex-shrink-0"
                >
