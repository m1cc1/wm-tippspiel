...
const ALL_48 = ['🇲🇽 Mexico','🇿🇦 South Africa','🇰🇷 South Korea','🇨🇿 Czechia','🇨🇦 Canada','🇧🇦 Bosnia & Herz.','🇵🇹 Portugal','🇨🇩 DR Congo','🇧🇷 Brazil','🏴󠁧󠁢󠁳󠁣󠁴󠁿 Scotland','🇲🇦 Morocco','🇭🇹 Haiti','🇺🇸 USA','🇵🇾 Paraguay','🇹🇷 Türkiye','🇦🇺 Australia','🇪🇸 Spain','🇪🇬 Egypt','🇦🇹 Austria','🇯🇴 Jordan','🇯🇵 Japan','🇹🇳 Tunisia','🇨🇴 Colombia','🇨🇮 Ivory Coast','🇩🇪 Germany','🇸🇦 Saudi Arabia','🇸🇪 Sweden','🇳🇿 New Zealand','🇳🇱 Netherlands','🇸🇳 Senegal','🇮🇷 Iran','🇪🇨 Ecuador','🇫🇷 France','🇳🇴 Norway','🇮🇶 Iraq','🇩🇿 Algeria','🇦🇷 Argentina','🇶🇦 Qatar','🇬🇭 Ghana','🇺🇿 Uzbekistan','🏴󠁧󠁢󠁥󠁮󠁧󠁿 England','🇭🇷 Croatia','🇵🇦 Panama','🇧🇪 Belgium','🇺🇾 Uruguay','🇨🇻 Cape Verde','🇨🇼 Curaçao','🇨🇭 Switzerland']
...
type ModalMode = 'closed'|'join-code'|'join-register'|'join-payment'|'login'
...
export default function HomePage() {
  ...
  function openLogin(){setModal('login');setLoginEmail('');setLoginPassword('');setLoginErr('')}
  ...
  async function handleRegister(e:React.FormEvent){
    ...
    setLoading(false);setModal('join-payment')
  }
  ...
  async function handleLogin(e:React.FormEvent){
    ...
    if(err){setLoginErr(err.message);setLoading(false);return}
    setLoading(false);closeModal();router.push('/dashboard');router.refresh()
  }

  const stepNum = modal==='join-code'?1:modal==='join-register'?2:3
  const isJoinFlow = modal.startsWith('join')

  const SHEET_STYLE = {background:'var(--bg)',borderRadius:'24px 24px 0 0',width:'100%',maxWidth:480,boxShadow:'0 -8px 40px rgba(26,24,20,0.25)',maxHeight:'95vh',overflowY:'auto' as const}
    ...
    <div style={{background:'var(--bg)',minHeight:'100vh'}}>
                  ...
                  <form onSubmit={handleLogin} style={{display:'flex',flexDirection:'column',gap:12}}>
                    <div>
                      ...
                      <input type="email" value={loginEmail} onChange={e=>setLoginEmail(e.target.value)} placeholder="you@example.com" required
                        className="input" style={{borderRadius:12,fontSize:14,padding:'13px 14px'}}/>
                    </div>
                    <div>
                    ...
                    {loginErr&&<div style={{background:'#fef2f2',border:'1px solid #fecaca',borderRadius:10,padding:'10px 14px',fontSize:13,color:'#991b1b'}}>{loginErr}</div>}
              ...
              {modal==='join-code'&&(
                <>
                  ...
                  <form onSubmit={checkCode} style={{display:'flex',flexDirection:'column',gap:12}}>
                    ...
                    <div style={{textAlign:'center',fontSize:13,color:'var(--text-faint)'}}>
                      Already registered? <button type="button" onClick={openLogin} style={{color:'var(--text)',fontWeight:700,background:'none',border:'none',cursor:'pointer',fontSize:13}}>Sign in</button>
                    </div>
                  </form>
                </>
              )}

              {/* ── JOIN STEP 2: REGISTER ── */}
              {modal==='join-register'&&(
                <>
                  ...
                  <form onSubmit={handleRegister} style={{display:'flex',flexDirection:'column',gap:10}}>
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
                      <div>
                        <div style={{fontSize:11,fontWeight:700,color:'var(--text-faint)',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:6}}>First name</div>
                        <input type="text" value={firstName} onChange={e=>setFirstName(e.target.value)} placeholder="Miro" required className="input" style={{borderRadius:10,fontSize:14}}/>
                      </div>
                      <div>
                        <div style={{fontSize:11,fontWeight:700,color:'var(--text-faint)',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:6}}>Last name</div>
                        <input type="text" value={lastName} onChange={e=>setLastName(e.target.value)} placeholder="Harasic" required className="input" style={{borderRadius:10,fontSize:14}}/>
                      </div>
                    </div>
                    <div>
                      <div style={{fontSize:11,fontWeight:700,color:'var(--text-faint)',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:6}}>Email</div>
        ...
        <div style={{maxWidth:1400,margin:'0 auto',padding:'0 20px',height:54,display:'flex',alignItems:'center',justifyContent:'space-between',gap:12}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:8}}>
            <select value={tz} onChange={e=>setTz(Number(e.target.value))}
              style={{fontSize:11,border:'1px solid var(--border)',borderRadius:100,padding:'6px 10px',background:'transparent',color:'var(--text-dim)',cursor:'pointer',fontFamily:'Inter Tight',fontWeight:500,maxWidth:130}}>
              {TZ_OPTIONS.map(o=><option key={o.offset} value={o.offset}>{o.label}</option>)}
            </select>
            <button onClick={openLogin} style={{fontSize:12,fontWeight:600,color:'var(--text-dim)',background:'none',border:'1px solid var(--border)',padding:'7px 14px',borderRadius:100,cursor:'pointer',transition:'all 0.2s'}}
      ...
      <div style={{borderBottom:'1px solid var(--border)',padding:'12px 0',overflow:'hidden',display:'flex',alignItems:'center',gap:12}}>
        ...
        <div style={{overflow:'hidden',flex:1}}>
          <div style={{display:'inline-block',animation:'ticker 40s linear infinite',whiteSpace:'nowrap',fontSize:12,color:'var(--text-dim)'}}>
            {['🇫🇷 France 2–1 Senegal · 78\'','Open to join — invite code needed','Winner takes 60% of prize pool','104 matches · up to 10 pts each','📰 Mbappé confirmed in France XI','📰 Record 5.8M tickets sold — biggest WC ever',
              '🇫🇷 France 2–1 Senegal · 78\'','Open to join — invite code needed','Winner takes 60% of prize pool','104 matches · up to 10 pts each','📰 Mbappé confirmed in France XI','📰 Record 5.8M tickets sold — biggest WC ever',
      ...
      <div className="page-wrap">
        ...
        <div style={{marginBottom:48}}>
          ...
          <h1 style={{fontFamily:'Bebas Neue',fontSize:'clamp(52px,11vw,140px)',lineHeight:0.9,letterSpacing:'-0.01em',color:'var(--text)',marginBottom:20}}>
            World Cup 2026<br/><span style={{color:'var(--beige-deep)'}}>Betting</span> Game
          </h1>
          <p style={{fontSize:15,color:'var(--text-dim)',maxWidth:440,lineHeight:1.65,marginBottom:24}}>
            Predict every match, earn points live with every goal, and battle for the prize pool. Built for people who love football and a little friendly competition.
          </p>
        ...
        {/* POINTS SYSTEM */}
        <div style={{marginBottom:48}}>
          <div style={{fontFamily:'Bebas Neue',fontSize:'clamp(36px,6vw,64px)',lineHeight:0.9,color:'var(--text)',marginBottom:16}}>
            How you <span style={{color:'var(--beige-deep)'}}>score</span>
          </div>
          <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:10}}>
            {[
              {n:'5',name:'Correct winner',desc:'Right outcome or draw',dark:false},
              {n:'3',name:'Goal difference',desc:'e.g. tip 2–0, actual 3–1',dark:false},
              {n:'1',name:'Exact team goals',desc:'1 pt per team you got right',dark:false},
